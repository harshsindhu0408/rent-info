  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const totalRevenue = rentals.reduce((sum, r) => sum + (r.finalAmountCollected || 0), 0);
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h1 className="text-2xl md:text-4xl font-bold text-gray-900 tracking-tight">Dashboard</h1>
          <p className="text-gray-500 mt-1 md:mt-2 text-sm md:text-lg">
            Welcome back! Here's your fleet overview for {format(new Date(), "MMMM yyyy")}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="sm:col-span-2 bg-gradient-to-br from-emerald-500 to-green-600 p-5 md:p-6 rounded-2xl md:rounded-3xl text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-3xl md:text-5xl font-bold mb-3 md:mb-4">₹{stats.totalRevenue.toLocaleString()}</p>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              <div className="bg-white/20 backdrop-blur px-2 md:px-3 py-1 md:py-1.5 rounded-lg flex items-center gap-1 md:gap-2 w-fit">
                {revenueChange >= 0 ? (
        ...
        {/* Active Rentals */}
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 md:p-6 rounded-2xl border border-amber-100">
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <div className="p-2 md:p-3 bg-amber-500 rounded-lg md:rounded-xl text-white">
              <Activity size={18} className="md:w-[22px] md:h-[22px]" />
            </div>
            <span className="text-amber-600 text-xs font-bold uppercase tracking-wider">Live</span>
          </div>
          <p className="text-3xl md:text-4xl font-bold text-amber-700">{stats.activeRentals}</p>
          <p className="text-amber-600 text-xs md:text-sm font-medium mt-1">Active Rentals</p>
        </div>

        {/* Pending Settlements */}
        <div className="bg-gradient-to-br from-rose-50 to-pink-50 p-4 md:p-6 rounded-2xl border border-rose-100">
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <div className="p-2 md:p-3 bg-rose-500 rounded-lg md:rounded-xl text-white">
              <AlertCircle size={18} className="md:w-[22px] md:h-[22px]" />
            </div>
            <span className="text-rose-600 text-xs font-bold uppercase tracking-wider">Action</span>
          </div>
          <p className="text-3xl md:text-4xl font-bold text-rose-700">{stats.pendingSettlements}</p>
          <p className="text-rose-600 text-xs md:text-sm font-medium mt-1">Pending Settlements</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <div className="bg-white p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-gray-500 text-xs md:text-sm font-medium">Fleet</span>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">{stats.totalCars}</p>
        </div>
        ...
        <div className="bg-white p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-gray-500 text-xs md:text-sm font-medium">Available</span>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-emerald-600">{stats.availableCars}</p>
        </div>
        ...
        <div className="bg-white p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-gray-500 text-xs md:text-sm font-medium">On Rent</span>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-orange-600">{stats.rentedCars}</p>
        </div>
        ...
        <div className="bg-white p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
            ...
            <span className="text-gray-500 text-xs md:text-sm font-medium">Completed</span>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-gray-900">{stats.completedRentals}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 md:px-6 py-4 md:py-5 border-b border-gray-100 flex justify-between items-center">
            <h2 className="text-base md:text-lg font-bold text-gray-900">Recent Rentals</h2>
            <Link
              to="/rentals"
              className="text-blue-600 text-xs md:text-sm font-medium hover:text-blue-700 flex items-center gap-1"
            >
              View All <ArrowUpRight size={12} className="md:w-[14px] md:h-[14px]" />
            </Link>
          </div>
          ...
          <div className="divide-y divide-gray-50">
            {recentRentals.map((rental) => (
              ...
              >
                <div className="flex items-center gap-3 md:gap-4 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs md:text-sm text-gray-500 truncate">
                      {rental.customer?.name} • {format(new Date(rental.startTime), "MMM d")}
                    </p>
                  </div>
                </div>
                <div className="text-right shrink-0 ml-2">
                  ...
                  <span
                    className={`inline-flex items-center px-1.5 md:px-2 py-0.5 rounded-lg text-[10px] md:text-xs font-medium ${rental.status === "Active"
                  ...
                  >
                    {rental.status === "Active" ? "Active" : rental.isSettled ? "Settled" : "Pending"}
                  </span>
                </div>
              </div>
            ))}
            {recentRentals.length === 0 && (
              <div className="px-6 py-8 md:py-12 text-center text-gray-500">
                <Car size={32} className="mx-auto mb-3 text-gray-300 md:w-[40px] md:h-[40px]" />
                <p className="text-sm md:text-base">No rentals yet. Create your first rental!</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 md:px-6 py-4 md:py-5 border-b border-gray-100">
            <h2 className="text-base md:text-lg font-bold text-gray-900">Top Cars by Revenue</h2>
          </div>
          ...
          <div className="p-3 md:p-4 space-y-2 md:space-y-3">
            {topCars.map((car, index) => (
              ...
              >
                <div
                  className={`w-7 h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center font-bold text-xs md:text-sm shrink-0 ${index === 0
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] md:text-xs text-gray-500">{car.count} rentals</p>
                </div>
                <p className="font-bold text-gray-900 text-sm md:text-base shrink-0">₹{car.totalCollected?.toLocaleString()}</p>
              </div>
            ))}
            {topCars.length === 0 && (
      ...
      {monthlyData.length > 0 && (
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 p-4 md:p-6">
          <h2 className="text-base md:text-lg font-bold text-gray-900 mb-4 md:mb-6">Monthly Revenue Trend</h2>
          <div className="flex items-end justify-between gap-2 md:gap-4 h-36 md:h-48 overflow-x-auto">
            {monthlyData.map((month) => {
              ...
              return (
                <div key={month._id} className="flex-1 min-w-[40px] md:min-w-[60px] flex flex-col items-center">
                  <div
                  ...
                  ></div>
                  <p className="text-[10px] md:text-xs text-gray-500 mt-2 font-medium">{month._id}</p>
                  <p className="text-[10px] md:text-xs text-gray-400">₹{(month.totalCollected / 1000).toFixed(0)}k</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        >
          <Calendar size={20} className="text-blue-600 mb-2 md:mb-3 group-hover:scale-110 transition md:w-[24px] md:h-[24px]" />
          <p className="font-bold text-gray-900 text-sm md:text-base">Manage Rentals</p>
          <p className="text-xs md:text-sm text-gray-500 mt-1 hidden sm:block">View and edit all rentals</p>
        </Link>
        <Link
          to="/cars"
        ...
        >
          <Car size={20} className="text-emerald-600 mb-2 md:mb-3 group-hover:scale-110 transition md:w-[24px] md:h-[24px]" />
          <p className="font-bold text-gray-900 text-sm md:text-base">Fleet</p>
          <p className="text-xs md:text-sm text-gray-500 mt-1 hidden sm:block">Add or update vehicles</p>
        </Link>
        <Link
          to="/reports"
        ...
        >
          <TrendingUp size={20} className="text-violet-600 mb-2 md:mb-3 group-hover:scale-110 transition md:w-[24px] md:h-[24px]" />
          <p className="font-bold text-gray-900 text-sm md:text-base">Reports</p>
          <p className="text-xs md:text-sm text-gray-500 mt-1 hidden sm:block">Analytics and insights</p>
        </Link>
        <div
        ...
        >
          <Plus size={20} className="mb-2 md:mb-3 group-hover:scale-110 transition md:w-[24px] md:h-[24px]" />
          <p className="font-bold text-sm md:text-base">Create Rental</p>
          <p className="text-xs md:text-sm text-gray-300 mt-1 hidden sm:block">Start a new booking</p>
        </div>
      </div>
    </div>
  );
