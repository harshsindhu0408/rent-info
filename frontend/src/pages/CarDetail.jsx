...
const CarDetail = () => {
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            const res = await api.patch(`/api/cars/${id}`, editForm);
            setCar(res.data);
            setIsEditing(false);
        ...
        return (
            <div className="flex justify-center items-center min-h-[50vh]">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-gray-500 font-medium text-sm">Loading vehicle...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] p-4">
                <div className="bg-white p-8 md:p-12 rounded-2xl shadow-sm text-center max-w-md w-full">
                    ...
                    <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-2">Vehicle Not Found</h2>
                    <p className="text-gray-500 mb-6 text-sm md:text-base">The vehicle you're looking for doesn't exist.</p>
                    <button
            ...
            <div className="max-w-6xl mx-auto mb-4 md:mb-8">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 md:gap-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold tracking-wide uppercase ${car.status === "Available"
                        ...
                        {isEditing ? (
                            <>
                                <button
                                    onClick={() => { setIsEditing(false); setEditForm(car); }}
                ...
                <div className="lg:col-span-2 space-y-4 md:space-y-6">
                        ...
                        <div className="relative z-10">
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4 md:mb-6">
                                <div>
                                    ...
                                    <h2 className="text-xl md:text-3xl font-bold text-gray-900">{car.model}</h2>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 pt-4 md:pt-6 border-t border-gray-100">
                                <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className="text-lg md:text-xl font-bold text-gray-900">₹{car.hourlyRate?.toLocaleString()}</div>
                                    )}
                                </div>
                                <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className="text-lg md:text-xl font-bold text-gray-900">₹{car.dailyRate?.toLocaleString()}</div>
                                    )}
                                </div>
                                <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                    <div className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5 md:mb-1">Expenses</div>
                                    <div className="text-lg md:text-xl font-bold text-gray-900">₹{totalExpenses.toLocaleString("en-IN")}</div>
                                </div>
                                <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                    <div className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5 md:mb-1">Last Service</div>
                                    <div className="text-sm md:text-base font-bold text-gray-900 truncate">{formatDate(car.lastServicedAt)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                            ...
                            <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Fuel</span>
                                </div>
                                {isEditing ? (
                                    <select
                                    ...
                                    <p className="text-sm md:text-base font-semibold text-gray-900">{car.fuelType || "—"}</p>
                                )}
                            </div>
                            ...
                            <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                ...
                                {isEditing ? (
                                    ...
                                    <p className="text-sm md:text-base font-semibold text-gray-900">{car.transmission || "—"}</p>
                                )}
                            </div>
                            ...
                            <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                ...
                                {isEditing ? (
                                    ...
                                    <p className="text-sm md:text-base font-semibold text-gray-900">{car.seatingCapacity || 5}</p>
                                )}
                            </div>
                            ...
                            <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                ...
                                {isEditing ? (
                                    ...
                                    <p className="text-sm md:text-base font-semibold text-gray-900">{car.color || "—"}</p>
                                )}
                            </div>
                            ...
                            <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                ...
                                {isEditing ? (
                                    ...
                                    <p className="text-sm md:text-base font-semibold text-gray-900">{car.year || "—"}</p>
                                )}
                            </div>
                            ...
                            <div className="bg-gray-50 p-3 md:p-4 rounded-lg md:rounded-xl">
                                ...
                                {isEditing ? (
                                    ...
                                    <p className={`text-sm md:text-base font-semibold ${car.status === "Available" ? "text-emerald-600" :
                                            car.status === "Rented" ? "text-amber-600" : "text-gray-600"
                                        }`}>{car.status}</p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 p-5 md:p-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                            {/* Insurance */}
                            <div className={`p-4 rounded-xl border-2 ${isExpired(car.insuranceExpiry) ? "bg-red-50 border-red-200" :
                                    isExpiringSoon(car.insuranceExpiry) ? "bg-amber-50 border-amber-200" :
                                        car.insuranceExpiry ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border-gray-200"
                                }`}>
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs font-medium text-gray-600">Insurance Expiry</span>
                                    {isExpired(car.insuranceExpiry) && <AlertTriangle size={14} className="text-red-500" />}
                                    {isExpiringSoon(car.insuranceExpiry) && !isExpired(car.insuranceExpiry) && <AlertTriangle size={14} className="text-amber-500" />}
                                    {!isExpired(car.insuranceExpiry) && !isExpiringSoon(car.insuranceExpiry) && car.insuranceExpiry && <CheckCircle size={14} className="text-emerald-500" />}
                                </div>
                                {isEditing ? (
                                    <input
                                        type="date"
                                        className="w-full bg-white rounded px-2 py-1.5 border border-gray-200 text-sm font-medium focus:ring-2 focus:ring-gray-200 outline-none"
                                        value={editForm.insuranceExpiry ? new Date(editForm.insuranceExpiry).toISOString().split("T")[0] : ""}
                                        onChange={(e) => setEditForm({ ...editForm, insuranceExpiry: e.target.value })}
                                    />
                                ) : (
                                    <p className={`text-base font-bold ${isExpired(car.insuranceExpiry) ? "text-red-700" :
                                            isExpiringSoon(car.insuranceExpiry) ? "text-amber-700" :
                                                car.insuranceExpiry ? "text-emerald-700" : "text-gray-500"
                                        }`}>{formatDate(car.insuranceExpiry)}</p>
                                )}
                            </div>

                            {/* PUC */}
                            <div className={`p-4 rounded-xl border-2 ${isExpired(car.pucExpiry) ? "bg-red-50 border-red-200" :
                                    isExpiringSoon(car.pucExpiry) ? "bg-amber-50 border-amber-200" :
                                        car.pucExpiry ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border-gray-200"
                                }`}>
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs font-medium text-gray-600">PUC Expiry</span>
                                    {isExpired(car.pucExpiry) && <AlertTriangle size={14} className="text-red-500" />}
                                    {isExpiringSoon(car.pucExpiry) && !isExpired(car.pucExpiry) && <AlertTriangle size={14} className="text-amber-500" />}
                                    {!isExpired(car.pucExpiry) && !isExpiringSoon(car.pucExpiry) && car.pucExpiry && <CheckCircle size={14} className="text-emerald-500" />}
                                </div>
                                {isEditing ? (
                                    <input
                                        type="date"
                                        className="w-full bg-white rounded px-2 py-1.5 border border-gray-200 text-sm font-medium focus:ring-2 focus:ring-gray-200 outline-none"
                                        value={editForm.pucExpiry ? new Date(editForm.pucExpiry).toISOString().split("T")[0] : ""}
                                        onChange={(e) => setEditForm({ ...editForm, pucExpiry: e.target.value })}
                                    />
                                ) : (
                                    <p className={`text-base font-bold ${isExpired(car.pucExpiry) ? "text-red-700" :
                                            isExpiringSoon(car.pucExpiry) ? "text-amber-700" :
                                                car.pucExpiry ? "text-emerald-700" : "text-gray-500"
                                        }`}>{formatDate(car.pucExpiry)}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl md:rounded-3xl shadow-xl p-5 md:p-8 text-white relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="mb-4">
                                ...
                                <p className="text-gray-400 text-xs mt-1">Total maintenance cost</p>
                            </div>
                            ...
                            <div className="pt-4 border-t border-gray-700">
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">{formatDate(car.lastServicedAt)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 p-5 md:p-6">
                        ...
                        {car.maintenanceHistory && car.maintenanceHistory.length > 0 ? (
                            <div className="space-y-2 max-h-[400px] overflow-y-auto">
                                {[...car.maintenanceHistory].reverse().map((record, idx) => (
                                    <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                                        <div className="min-w-0 flex-1">
                                            <p className="font-medium text-gray-900 text-sm truncate">{record.description}</p>
                                            <p className="text-[10px] text-gray-500">{formatDate(record.date)}</p>
                                        </div>
                                        <p className="font-bold text-gray-900 text-sm ml-3">₹{record.amount?.toLocaleString("en-IN")}</p>
                                    </div>
                                ))}
                            </div>
                        ) : (
